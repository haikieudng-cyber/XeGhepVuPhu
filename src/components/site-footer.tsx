    ...
    <footer className="border-t border-slate-200 bg-white text-slate-600">
      <div className="mx-auto grid max-w-6xl gap-8 px-4 py-10 sm:px-6 md:grid-cols-2 lg:grid-cols-4 lg:px-8">
        <div className="space-y-4">
          ...
          <div className="flex gap-2">
            {["f", "Zalo", "Tik", "You"].map((item) => (
              <span
                className="grid h-8 w-8 place-items-center rounded-full bg-[var(--color-navy-900)] text-[10px] font-black text-white"
                key={item}
              >
                {item.slice(0, 1)}
              </span>
        ...
        <div>
          ...
          <div className="flex gap-2">
            {["f", "Z", "T", "Y"].map((item) => (
              <span className="grid h-9 w-9 place-items-center rounded-full border border-slate-200 bg-white text-xs font-black text-[var(--color-navy-900)]" key={item}>
                {item}
              </span>
            ))}
          </div>
